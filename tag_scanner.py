...
def parse_color_code(text: str) -> Optional[Dict[str, str]]:
    """
    Extract color code from OCR text.
    Looks for known Skechers color codes.
    
    Args:
        text: OCR extracted text
        
    Returns:
        Dictionary with color_code and color_name, or None
    """
    # Convert text to uppercase for matching
    text_upper = text.upper()
    
    # Look for color field patterns
    # Pattern 1: "Color" or "CLR Code" followed by color code
    ...
    # Pattern 2: Search for any known color code in the text
    for color_code in sorted(BASE_COLOR_CODES.keys(), key=len, reverse=True):
        # Use word boundaries to avoid partial matches
        if re.search(r'\b' + re.escape(color_code) + r'\b', text_upper):
            return {
                'color_code': color_code,
                'color_name': BASE_COLOR_CODES[color_code]
            }
...
def scan_skechers_tag(image_data: bytes) -> Dict:
    ...
    try:
        ...
        # Parse color code
        color_info = parse_color_code(text)
        
        if not style_number and not color_info:
            return {
                'success': False,
        ...
        result = {
            ...
            'color_code': color_info['color_code'] if color_info else None,
            'color_name': color_info['color_name'] if color_info else None,
            'raw_text': text
        }
